...
static std::string GetTimeOfDayDesc()
{
    ...
    static const char* timePools[][11] = {
        /* 清晨 4-7 */  { "天刚蒙蒙亮，雾气还没散", "晨露打湿了我的靴子", "早起的人有收获，不是吗", "太阳刚刚露出地平线", "空气真新鲜，清晨最好了", "鸟都还没起床呢", "第一缕阳光总是最美的", "又是一个新开始", "清晨的风让人精神一振", "趁还没热，赶紧赶路", "湖面上的薄雾真好看" },
        /* 上午 7-12 */ { "早上好旅行者，今天精力充沛吗", "太阳升起来，暖和多了", "上午是干活的最好时间", "阳光还不算刺眼", "精力充沛，今天争取多走一段", "早上的集市刚开张", "这个时间最适合赶路", "上午的光线真舒服", "今天任务排满了", "上午的猎物最多", "趁着凉快把活干了" },
        /* 下午 12-18 */{ "太阳正当头，晒得人发晕", "走到下午腿都酸了", "中午吃太多了有点困", "这日头可真毒", "下午的时光最慵懒", "该找个阴凉地方打个盹", "下午的猎物不好找", "热得我盔甲都不想穿了", "下午的阳光让一切都慢下来", "走了半天了，该歇歇了", "真想来杯凉水" },
        /* 黄昏 18-21 */{ "天快黑了，得找个地方扎营", "黄昏的余光真美", "一天又要结束了", "该准备篝火了", "趁着还有光再走一段", "黄昏是狼群出没的时间，小心", "夕阳把天边染成了金色", "又是忙碌的一天", "晚风开始吹起来了", "满天霞光，明天该是个好天气", "我喜欢黄昏的宁静" },
        /* 深夜 21-4 */ { "好黑……星星真多", "该守夜了，你先睡吧", "深夜的荒野不太平", "篝火烧得噼啪响", "月亮又圆又亮", "夜里什么都看不清", "听到远处的狼嚎了吗", "冷……靠近篝火一点", "夜里的森林像另一个世界", "星空下思考人生", "这个点还不睡会倒霉的" },
    };
...
static std::string GetWeatherDesc(Player* bot)
{
    ...
    if (hour >= 22 || hour < 6)
    {
        if (roll < 20) return "夜里下起了雨";
        if (roll < 35) return "夜里起了雾";
        return "夜色不错";
    }
    if (roll < 10) return "下雨了";
    if (roll < 20) return "起雾了";
    return "天气不错";
}
...
static std::string GetRumorFlavor(uint32 zoneType)
{
    ...
    static const char* rumorPools[6][11] = {
        { "听说荆棘谷有个失踪的海盗宝藏", "暮色森林的乌鸦最近特别多", "银松森林的狼人又多了几个", "费伍德的萨特在密谋什么", "灰谷的精灵最近不太友善", "森林里有只白鹿，看到的人都会交好运", "有猎人说在林子里看到了龙", "古树在低语，谁听懂了", "蘑菇圈是妖精的舞池，别踩", "森林里住着一位隐士", "昨晚林子里有绿光在闪" },
        { "听说贫瘠之地的钢鬃野猪人又在搞事了", "十字路口的补给车队被劫了", "莫高雷的灵魂之泉最近不太平静", "西部荒野的迪菲亚要卷土重来", "赤脊山那边有个黑龙的传说", "平原上的斑马群在迁徙", "剃刀岭出了个了不起的新兵", "有人看到了半人马在集结", "草原上的日落会让人流泪……传说而已", "试炼谷深处藏着古老的试炼武器", "野猪人山洞里有宝藏" },
        { "传说冬泉谷深处有永冻的财宝", "铁炉堡的矿工挖通了新矿脉", "洛克莫丹的水坝下面有东西在动", "冬天会有雪人下山，真的", "丹莫罗的矮人酿了新酒", "冰雪女王的故事……都是吓小孩的", "奥达曼下面有沉睡的泰坦造物", "暴风雪之夜出现了幽灵旅队", "冻住的湖里冻着一只龙", "听说冬泉火酒就是用永冻之地的火焰酿的", "雪地里发现了古老的符文" },
        { "塔纳利斯的废墟下埋着巨魔帝国", "沙子里挖出了虫卵，当心", "加基森的拍卖行有人卖假货", "传说流沙之战时这里死了上万人", "千针石林下面真的有千根石针", "希利苏斯的暮光教徒在找什么", "沙漠里有一个永远不干涸的泉眼", "铜龙说有一种沙漏能让人回到过去", "有人看见时光龙飞过沙漠", "废旧渡口那边有幽灵船出没", "沙暴中隐约看到了巨龙的骨架" },
        { "悲伤沼泽里有一座消失的神庙", "诅咒之地是兽人第一次踏进的地方", "安戈洛的水晶能用来做顶级附魔", "沼泽里有一种花，闻了会做梦", "沼泽深处住着一个老巫婆", "黑暗之门附近总有恶魔在徘徊", "有人说安戈洛是泰坦的实验场", "沼泽里的雾气会让人迷路到死", "泥沼里埋着上古之战的遗物", "失落的神庙里藏着阿塔哈卡的血", "腐败之水中似乎有什么在沉睡" },
        { "艾萨拉的海底有娜迦的宫殿", "荆棘谷的海盗藏宝图在黑市上卖", "菲拉斯的遗迹里有一扇打不开的门", "提瑞斯法海岸的渔夫捞上来过骷髅", "传说大海深处有沉没的城市", "蓝龙军团为什么守着艾萨拉", "海妖的歌声……能让人发疯", "双子皇帝的石碑刻着远古诅咒", "有人在海岸看到过美人鱼？胡扯", "海盗们说荆棘谷有座骷髅岛", "满月之夜，海岸边传来鳍人的战歌" },
    };

    return rumorPools[zoneType - 1][urand(0, 10)];
}
...
static std::string BuildOutdoorFlavor(Player* bot, Player* source, uint32 zoneId, uint32 mapId)
{
    ...
    // zone 主题池
    static const char* zonePools[6][24] = {
        { "林子里有动静……小心点", "这些古树比联盟部落还老", "树冠密得看不到天", "闻到了吗？松针和泥土的味道", "这边的小径通向哪来着", "林间的光斑真好看", "嘘……听到猫头鹰叫了吗", "蜘蛛网太多了，烦死了", "枯叶踩着哗哗响", "这棵树得有上千年了吧", "树下有蘑菇，能吃吗", "林间空地……适合扎营", "树枝上挂着什么东西", "别走散了，林子深", "松涛声真舒服", "这林子让我有点发毛", "暗影在树间摇曳……", "腐化的气息弥漫在空气中", "萨特在暗处窥视着我们", "碧火在树梢间闪烁", "被诅咒的树林永不安宁", "这里曾是精灵的圣地", "银松的暗影永无止境", "狼人的嚎叫还回荡在耳边" },
        { "烈日当头，连风都是热的", "草原一望无际，让人心潮澎湃", "这片土地很适合远征", "野猪人又闹事了", "十字路口的商队到了吗", "贫瘠之地的日落是最美的", "真想在草原上跑一跑", "看着天际线，觉得自己很渺小", "麦田里的稻草人……有点诡异", "迪菲亚盗贼真是烦人", "湖畔镇的鱼还不错", "山那边的黑石兽人不太安分", "在草原上深呼吸……舒服", "斑马群跑过去了", "这道路修得还算平整", "试炼谷的新兵越来越多了", "兽人的战鼓声在回荡", "雷霆崖在远处若隐若现", "莫高雷的风永远那么温柔", "这片平原，是战士的故乡", "半人马的蹄声从远方传来", "贫瘠之地的水源就是生命", "西部荒野的风车还在转吗", "赤脊山的石堡巍然耸立" },
        { "冻死我了……来杯麦酒暖暖身子", "雪地里踩下去咯吱咯吱的", "丹莫罗的雪从不融化", "铁炉堡的锻炉声还听得见吗", "雪下得太大，脚印都被盖了", "冬泉谷的枭兽又在嚎了", "这地方冷得连火都打不着", "雪崩了吗？刚才听到了轰隆声", "洛克莫丹的水坝真壮观", "穴居人又来捣乱了，烦", "雪地里看到熊的脚印", "再往前走就是冰川了", "冻土硬得像石头", "奇美拉的尖叫划破天际", "这辈子最讨厌的就是暴风雪", "来口矮人烈酒就好了", "雪反射的阳光刺眼得很", "这温度能冻掉耳朵", "快走吧，风越来越大了", "冰面上走路小心点", "永冻之地的传说代代相传", "铁炉堡的灯火在风雪中闪烁", "雪人毛皮能卖个好价钱", "丹莫罗的麦酒是全艾泽拉斯最好的" },
        { "呸，满嘴沙子", "这鬼地方连水都找不到", "沙漠里的夜晚冷得要死", "影子的方向……那边是东吧", "加基森的地精商人就是一群骗子", "风蛇在空中盘旋，当心", "铜龙在时之穴守着什么", "千针石林的峡谷太壮观了", "徒步穿越千针真是要命", "沙漠里的水比金子还贵", "前面有绿洲吗？渴死了", "虫群的嗡鸣声越来越大", "其拉虫人在沙下蠢蠢欲动", "暮光教徒的营地就在不远处", "希利苏斯的风……带着塑像的低语", "甲虫之锣还没敲响", "沙暴又来了，快找掩体", "废墟里或许埋着什么好东西", "这沙漠永远走不到尽头", "废旧渡口那边有海盗出没", "加基森的拍卖行什么稀奇玩意儿都有", "热砂港的海风也吹不凉这沙漠", "时光之穴的龙族让人敬畏", "千针石林的升降梯真刺激" },
        { "泥泞的路真难走", "沼泽在冒泡……离远点", "失落的神庙就在前面", "悲伤沼泽的雾从不散开", "空气中弥漫着腐臭", "恶魔的能量扭曲了这片大地", "黑暗之门就在不远处", "诅咒之地的天空总是血红色的", "这片泥沼吞噬了太多英雄", "恐龙！那边有恐龙！", "安戈洛的水晶……闪耀着奇异的光", "探险队的营地还远吗", "沼泽里有什么东西在动", "这片土地的魔力还未消散", "每一步都在陷进泥里……烦", "火山口喷出的热气烫得吓人", "失落的神庙里……不该进去的", "诅咒之地的每一寸土地都在低语", "安戈洛环山，奇迹与危险并存", "沼泽深处传来了低沉的吼声", "恐龙足迹还新鲜着……小心", "腐败的气息越来越浓了", "安戈洛的火羽山隔三差五就喷发", "诅咒之地的恶魔不眠不休" },
        { "海风带着咸味，真舒服", "娜迦的废墟就在不远处", "蓝龙盘旋在艾萨拉上空", "坠星海滩的沙子是银色的", "荆棘谷的丛林密不透风", "海盗！有海盗船！", "听到了吗？古拉巴什的鼓声", "巨魔的营地就藏在林子里", "潮湿的海风吹得盔甲生锈", "这海岸线上不知埋了多少宝藏", "海鸥的叫声吵死了", "双子皇帝的石碑刻着什么", "菲拉斯的食人魔……真够笨的", "遗迹里藏着远古的秘密", "幽暗城外的蝙蝠在盘旋", "被遗忘者的灰色皮肤在月光下更加苍白", "提瑞斯法的树林永远阴森", "海岸的雾气让我看不清路", "海水拍打礁石的声音真催眠", "在岸边走，当心涨潮", "艾萨拉的枫叶红得像火", "荆棘谷的竞技场今天又有决斗了", "藏宝海湾的地精商会日进斗金", "菲拉斯的高塔在迷雾中若隐若现" },
    };
    ...
    auto getZoneText = [&](Player* b, Player* s, uint32 zid, int) -> std::string {
        ...
        if (zt >= 1 && zt <= 6)
            return zonePools[zt - 1][urand(0, 23)];
        return defaultPool[urand(0, 23)];
    };
    ...
    if (zoneType >= 1 && zoneType <= 6)
        return zonePools[zoneType - 1][urand(0, 23)];
    return defaultPool[urand(0, 23)];
}
